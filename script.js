...
function showAppDetails(appId) {
    ...
    // 生成容器信息部分
    const containerInfoSection = isUserApp && app.containerInfo ? `
        <div class="detail-section">
            <h4><i class="fas fa-server"></i> 容器信息</h4>
    ...
    modalBody.innerHTML = `
        <div class="app-detail-info">
            <div class="detail-section">
                ...
                ${isUserApp ? `<p><strong>状态：</strong><span class="health-${app.status}">${app.status === 'running' ? '运行中' : '已停止'}</span></p>` : ''}
            </div>
            ...
            <div class="detail-section">
                ...
                ${isUserApp ? `<p><strong>添加时间：</strong>${new Date(app.addedAt).toLocaleString()}</p>` : ''}
                ${isUserApp ? `<p><strong>最后使用：</strong>${new Date(app.lastUsed).toLocaleString()}</p>` : ''}
            </div>
